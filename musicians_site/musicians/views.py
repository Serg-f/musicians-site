...
class ShowIndex(MixinData, ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        user_dict = self.get_user_context(title='Главная страница')
...
class AddArticle(LoginRequiredMixin, MixinData, CreateView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        context = super().get_context_data()
        user_dict = self.get_user_context(title='Добавить статью')
        context.update(user_dict)
        return context
...
class Contact(MixinData, FormView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        context = super().get_context_data()
        user_dict = self.get_user_context(title='Обратная связь')
        context.update(user_dict)
        return context
...
class RegisterUser(MixinData, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        user_di = self.get_user_context(title='Регистрация')
...
class LoginUser(MixinData, LoginView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        user_di = self.get_user_context(title='Авторизация')
        context.update(user_di)
        return context
